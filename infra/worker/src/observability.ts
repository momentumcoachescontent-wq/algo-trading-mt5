...
export function normalizeTechnicalSignalStatus(body: Payload): string {
  ...
  if (toText(body.block_reason)) return "BLOCKED";
  const h4Signal = toNumber(firstDefined(body.filtered_h4_signal, body.h4_signal));
...
export function normalizeDecision(body: Payload): string {
  ...
  const blockReason = toText(body.block_reason);
...
export function normalizeSignalReasons(body: Payload): NormalizedReasons {
  const scope = normalizeExecutionScope(body);
  const blockReason = toText(body.block_reason);
  const generalReason = toText(body.reason);
  const decision = normalizeDecision(body);

  let primarySignalReason = toText(
    firstDefined(body.primary_signal_reason, body.signal_reason),
  );
  let governanceGuardReason = toText(
    firstDefined(body.governance_guard_reason, body.guard_reason),
  );
  let executionDeniedReason = toText(
    firstDefined(
      body.execution_denied_reason,
      scope.reason,
      body.scope_reason,
...
export function resolveSignalEvalId(body: Payload): string {
  ...
  const canonical = [
    ...
    toText(body.strategy_variant) ?? "unknown-variant",
    toText(firstDefined(body.bar_h4, body.eval_bar_time, body.eval_time)) ?? "unknown-time",
    toText(firstDefined(body.direction, body.signal_direction, body.action)) ?? "no-direction",
...
export function buildSignalObservability(body: Payload): SignalObservability {
  ...
  return {
    ...
    strategyVariant: toText(body.strategy_variant),
    ...
    barH4: toText(firstDefined(body.bar_h4, body.eval_bar_time)),
...
export function normalizeTradeIdentity(body: Payload, event: TradeEvent): TradeIdentity {
  ...
  const orderTicket = firstDefined(
    body.order_ticket,
    body.order_id,
    event === "trade_open" ? body.ticket : null,
  ) as string | number | null;
  const dealTicket = firstDefined(
    body.deal_ticket,
    body.deal_id,
    event === "trade_close" ? body.ticket : null,
  ) as string | number | null;
  ...
  if (event === "trade_open") {
    if (positionId && explicitSignalEvalId) linkStatus = "LINKED_SIGNAL_POSITION";
    else if (positionId) linkStatus = "POSITION_ONLY_NO_SIGNAL_LINK";
    else if (explicitSignalEvalId) linkStatus = "SIGNAL_LINK_MISSING_POSITION_ID";
    else linkStatus = "LEGACY_OPEN_IDENTITY_INCOMPLETE";
  } else {
    linkStatus = positionId ? "CLOSE_MATCH_BY_POSITION" : "UNMATCHED_CLOSE_IDENTITY";
  }

  return {
    ticket: event === "trade_open" ? orderTicket : dealTicket,
    orderTicket,
    dealTicket,
    positionId,
    signalEvalId: explicitSignalEvalId,
    executionMode: scope.mode,
    strategyVariant: toText(body.strategy_variant),
    guardVersion: toText(body.guard_version),
    magicNumber: toNumber(firstDefined(body.magic_number, body.magic)),
